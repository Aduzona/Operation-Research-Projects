---
title: "Synchronized lot sizing with capacity constraints"
author: "GroupB"
date: "14/08/2020"
output:
...
#par(mfrow=c(1,1))
{ 
  plot(x.val,round(y.vec.no.const), xlab ="Material ID", ylab = "total cost", type = "p", lwd =2, ylim=c(12,max(y.vec.no.const)), xaxp=c(1, 10, 1) )
  axis(1, 1:62)
axis(2, 1:62)
...
kable(tmp,format = "pandoc",caption = "selected items")
```
...
```{r}
model<-MIPModel() %>%
  add_variable(x[p],p=1:9,type="integer",lb=0,ub=32)%>%
  set_objective(sum_expr(x[p],p=1:9),"min")%>%
  add_constraint(sum_expr(a_ip[i,p]*x[p],p=1:9)>=ld[i],i=1:3)
...
```{r}
solution <- get_solution(result, x[p])%>%
  filter(value>0)
...
```{r}
opt_patterns<-as.data.frame(rbind(a_ip[,solution$p[1]],a_ip[,solution$p[2]]), colnames(c("200","400","600")))
opt_patterns
```
...
paste("Total of",result$objective_value,"rack_levels")
```
...
1. rack_total_width which was  56,000 mm, looking at the 9 patterns all of which have 150 mm in waste, meaning  all 32 levels will have 150 mm waste each. Therefore $56,000 - (150 *32) = 51,200$ thereby changing rack total width 51,200.
...
fitted.pattern <- opt_patterns#patterns[3:4,]
...
kable(product_data,caption = "item 4", format = "pandoc")
...
#[,1:11]
kable(continous.df,caption = paste("Continous Review Inventory  Policy item",item), row.names = T, escape = F,format = "pandoc")
```
...
#[,1:4]
kable(sq.summary,caption = "Continous Inventory SQ Policy",format = "pandoc")
```
...
The table above is Item 17 cycle time. Reordering points of this policy is Zero or near zero for all items. 
...
kable(so.mat.df,caption = "Items and samples",format = "pandoc")
