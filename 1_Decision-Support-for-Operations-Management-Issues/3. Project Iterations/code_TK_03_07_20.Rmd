...
M= 32 levels,K= 9 patterns $x_{m,k}$  consist of a vector of decision variables with $(M \cdot K)$ 288 elements $X= ( x_{11},x_{12},..x_{1,32},....x_{9,32} )$. There are $ J+M= 33$ constraints. Thus the right hand ride vector $b=(\vec J,1_1,...1_{32})$. Correspondingly, the constraint matrix $A$ has dimension $33 \times 288$ and looks as follows:
...
b.vec <- c(ld,rep(1,n.k)) # right hand side vector 

#objective coeficients
c.vec <- c(rep(1,(n.m*n.k)))
  
const.vec <- c(rep(">=",length(ld)), rep("<=",n.k)) # vector with constraint directions 

vtype.vec <- rep("B",length(x.vec)) # vector with variable types ("B" binary ) 


A.mat <- matrix(0, ncol = n.m*n.k, nrow = (3+n.k) )# initialize constraint matrix
...
for (i in 1:32) {
  while(tuple.id < 13){
  A.mat[tuple.id,grepl(nam.K[i],colnames(A.mat))]<- 1
