...
kable(product_data[1:5,c(2,3,12:ncol(product_data))],format = "pandoc")
...
kable(cycle.df[1:6,1:3], row.names = T, format = "pandoc")
```
...
kable(cyc[,1:4],caption = "cycle time for each item", format = "pandoc")
```
...
colnames(cyc)<-1:62
rownames(cyc)<-c("M","days")
kable(cyc[,1:10],caption = "cycle time for each item", format = "pandoc")
```
...
kable(period.df[,1:11],caption = "Periodic Inventory TS Policy", row.names = T, escape = F,format = "pandoc")
```
...
kable(ts.summary[,1:4],caption = "Periodic Inventory TS Policy",format = "pandoc")
...
cycle times for items 17 and 18 where assigned 37days, while that of items 50 is 6 days from 7days

```{r}
product_data$cycle_time_in_days[c(17,18)]<-37
product_data$cycle_time_in_days[c(50)]<-6
```
...
colnames(m.mat)<-c("sum",c(1:62))

kable(m.mat[,1:8], row.names = T, escape = F,format = "pandoc")
```
