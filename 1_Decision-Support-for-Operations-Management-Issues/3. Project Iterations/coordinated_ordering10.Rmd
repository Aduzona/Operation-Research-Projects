...
$$lane_i=\frac {q_i}{n_i}$$ 
$$lane_i=q_i \cdot \frac {b_i^{-1}}{rack_{length}}=214 \cdot \frac {297}{6000}=10.593= 11 \space lanes $$
...
$$214 \cdot \frac {396 \cdot 297}{6000}=4194.828 $$
$$ b_i= \frac {4194.828}{10.593}=396$$
...
obj.fun(copt_sol$solution)#202279.9
obj.fun(floor(copt_sol$solution))#205079.8 rounded q values


```


*  *Total costs with no Capacity  constraint Optimized*
This is using EOQ.min
```{r, include=FALSE, cache=F}
...
$$ subject \space to: \quad \quad \quad \quad \quad \quad \quad \quad \quad$$
...
$$ct= \frac {q}{yb_i}= \frac  {m_i \cdot B \cdot y_i}{yb_i} $$
$$ ct= \frac {18}{1.2}=15days$$
...
2. Inventory quatlity ratio $IQR_i= \frac {to_i}{s_i} \rightarrow $ the closer to 1, the better.
...
so.df <- data.frame(rbind(product_data$eoq.max,copt_sol$solution))
...
#floor(product_data$cycle_time_in_days[i])
