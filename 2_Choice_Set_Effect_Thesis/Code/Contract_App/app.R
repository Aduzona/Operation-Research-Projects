...
select_contract<- function(oc,ce){
  ...
  contract_low_high<-contract_low_high%>%filter(b=="b1")%>%inner_join(contract_low_high%>%filter(b=="b2"),by=c("Z"="Z","K"="K"))%>%select(K,Z,v.x,v.y,r.x,r.y)%>%`colnames<-`(c("$K_j$","$Z_l$","$Buyer (??=-1)$", "$Buyer(??=1)$","$Supplier(??=-1)$","$Supplier(??=1)$"))
...
select_contract2<- function(oc,ce){
  ...
  contract_low_high<-contract_low_high%>%filter(b=="b1")%>%inner_join(contract_low_high%>%filter(b=="b2"),by=c("Z"="Z","K"="K"))%>%select(K,Z,v.x,v.y,r.x,r.y)
  #ξ %>%kable()
...
ui <- dashboardPage(
  dashboardHeader(title = "Choice Set Effect on Contract in Supply Chain",
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "Offered_Contracts",
              helpText("Please type in 1 to 8"),
              box(numericInput(inputId = "Offered_Contract",
                               label = "Offered Contract:",
                               min = 1,
                               max = 8,
                               value = 4)),
              box(numericInput(inputId = "Offered_Contract2",
                               label = "Offered Contract with Card Attribute and Profit=1:",
                               min = 1,
                               max = 8,
                               value = 4)),
              box(numericInput("Estimate",
                               "Profit Estimate",
                               min=1,
                               max=9,
                               value=1) ),
              box(numericInput("Estimate2",
                               "Contract Estimate",
                               min=1,
                               max=9,
                               value=1) ),
              #selectInput(inputId = "graph",
                          #label = "Choice Set Effect",
                          #choices =list("No Choice Cardinality","Choice Cardinality")),
              h5("K(capacity), Z(reservation Fee), b1(Profit for low type buyer),b2(Profit for high type buyer),S(supplier)"),
              
              fluidRow(box(fluidRow(column(12,DT::dataTableOutput("table_menu")))),
              box(fluidRow(column(12,DT::dataTableOutput("table_menu2"))))),
              box(DT::dataTableOutput("Supplier_Profit")),
              box(DT::dataTableOutput("Supplier_Profit2"))
...
server <- function(input,output,session){
  ...
  output$table_menu <- DT::renderDataTable(DT::datatable({
      ...
      data2<- data.frame(select_contract(input$Offered_Contract,input$Estimate))
      colnames(data2)<- c("K","Z","b1","b2","S(b1)","S(b2)")
  ...
  output$table_menu2 <- DT::renderDataTable(DT::datatable({
      ...
      colnames(data2_2)<- c("K","Z","b1","b2","S(b1)","S(b2)")
  ...
  output$Supplier_Profit<-DT::renderDataTable(DT::datatable({
    #if(input$graph=="No Choice Cardinality"){
      Data_profit<- read.xlsx("Existing_irrational_12_p1.xlsx",sheet = 1)
      data3<-Data_profit%>% filter(Offered_Contract==input$Offered_Contract & Profit_Estimate== input$Estimate)%>% select(Supplier_Profit,N0_of_Contract)
  ...
  output$Supplier_Profit2<-DT::renderDataTable(DT::datatable({
    #if(input$graph=="Choice Cardinality"){
